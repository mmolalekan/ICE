      ...
      <div className="container mx-auto px-10 py-16  flex flex-col gap-8 w-full">
        <section className="flex flex-col md:flex-row gap-8">
          <div className="w-full flex flex-col items-center justify-center gap-12 h-full">
            <p className="text-xl text-justify text-cyan font-light px-2">Feel free to talk to our Business Department at any time. Please be patient while waiting for response.</p>
            <ul className="flex flex-col gap-4 items-start justify-center w-full">
              {info.map((item, index) => {
                return <li key={index} className="flex items-center gap-2">
                  <div className="w-[45px] h-[45px] bg-cyan rounded-full flex items-center justify-center">
                    <FontAwesomeIcon icon={item.icon} size="1x" className="text-white" />
                  </div>
                  <h3 className="font-light italic">{item.description}</h3>
                </li>
              })}
            </ul>
          </div>

          <form action="" className="flex flex-col gap-5 w-full bg-cyan rounded-tl-[48px] rounded-br-[48px] items-center justify-center px-8 py-12">
            <input className="bg-white rounded-2xl px-5 py-3 w-full max-w-[560px] text-cyan" type="text" placeholder="Name*" required />
            <input className="bg-white rounded-2xl px-5 py-3 w-full max-w-[560px] text-cyan" type="email" placeholder="Email*" required />
            ...
            <button className="border px-5 py-1 w-3/5 bg-cyan text-white uppercase text-[20px] font-bold rounded-full hover:bg-black/50">Submit</button>
          </form>
